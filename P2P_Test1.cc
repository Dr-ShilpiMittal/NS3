...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  while ( socket -> Recv() )
  {
    ...
    // std::cout << "Received Data Packet by node0:"<< destinationHeader.GetData ()<<std::endl;
...
//  Ptr<Socket> source = Socket::CreateSocket (terminals.Get (3), tid);
//  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("255.255.255.255"), 80);
//  source->SetAllowBroadcast (true);
//  source->Connect (remote);
...
// }
 ...
 flag=flag+1;
  ...
  AnimationInterface anim ("P2P_test.xml"); 
  Simulator::Run ();
  Simulator::Destroy ();
  return 0;
 }
